//*****************************************************************************
//
// usb_dev_serial3.c - Main routines for the USB CDC serial example.
//
// Deze versie gebruikt USB CDC voor een "echo" applicatie.
// Te gebruiken door middel van een seriele verbinding over een USB connectie
//
//*****************************************************************************
...
//*****************************************************************************
//
// ECHO for echo-ing of received characters though the USB CDC device
//
//*****************************************************************************
//#define ECHO
...
// todo aanpassen op nieuwe situatie; dit is specifiek voor UART
...
//*****************************************************************************
//
// Take as many bytes from the receive buffer as we have space for and move
// them back into the transmit buffer.
//
//*****************************************************************************
static void USBEchoTransmit()
{
    uint32_t ui32Read;
    uint8_t ui8Char;
    ...
    {
        //
        // write the character to the USBBufferTransmit
        //
...
SetLineCoding(tLineCoding *psLineCoding)
{
    ...
    //
    // Set the UART mode appropriately. Is now redundant!
    //
...
{
    uint32_t ui32Config = DEFAULT_UART_CONFIG;
    //uint32_t ui32Rate;
...
// to \b false.
//
// Break signals the start of a new packet
//
//*****************************************************************************
/*
static void
          ...
          void *pvMsgData)
{
    ...
    switch(ui32Event)
    {
        ...
        // process. The actual number of bytes in the UART FIFO is not
        // important here, merely whether or not everything previously
        // sent to us has been transmitted.
        //
...
main(void)
{
    ...
    while(1)
    {
        ...
        //
        if(ui32TxCount != g_ui32USBTxCount)
        {
            //
            ...
            //
            ui32TxCount = g_ui32USBTxCount;
        }

        //
        // Has there been any receive traffic since we last checked?
        //
        if(ui32RxCount != g_ui32USBRxCount)
        {
            //
